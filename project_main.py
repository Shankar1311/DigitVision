...
def start_cv(model):
    ...
    while True:
        ...
        if startInference:
            ...
            # Normalize the image (like how you trained the model)
            iconImg = iconImg / 255.0  # Normalize to range [0, 1]

            # Ensure the image has the correct shape (28, 28, 1)
            iconImg = np.expand_dims(iconImg, axis=-1)  # Add the channel dimension
...
def main():
    ...
    model = None
    try:
        model = tf.keras.models.load_model('model.keras')  # Changed to .keras
        ...
        print("saving model...")
        model.save('model.keras')  # Changed to .keras
